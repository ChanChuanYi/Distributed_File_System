    ...
    while(1)
    {
        ...
        if(child==0){
            ...
            while(read(connfd,recv,LINE) > 0){
                ...
                //detemine which function with client input command
                ...
                else if(strncmp("STOR",command,strlen(command))==0){
                    
                    data_socket=create_data_socket(IP,port);
                    read_file(connfd,data_socket,filename);
                    close(data_socket);
                }
                else if(strncmp("LIST",command,strlen(command))==0){
                    data_socket=create_data_socket(IP,port);
                    List_Direct(connfd,data_socket,filename,directory);
                    close(data_socket);
                }

                else if(strncmp("ABOR",command,strlen(command))==0){
                    printf("%s\n","Exiting FTP connection");
                    char reply[LINE];
                    snprintf(reply,sizeof(reply),"221 Goodbye");
                    write(connfd,reply,strlen(reply));
                    break;
                }
                else{
                    printf("%s %s %s: Invalid Command\n",command,filename,directory);
                    continue;
                }   
